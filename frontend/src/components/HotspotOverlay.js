...
const HotspotOverlay = ({ hotspot, onClose }) => {
  ...
  // Adjust the x and z coordinates of the overlay based on the hotspot's x position
  const adjustedX = x > 0 ? x - 2 : x + 2;
  const adjustedZ = x > 0 ? z + 1 : z - 1;
  
  // Adjust y coordinate
  const adjustedY = y + 0.5; // Slightly above the hotspot

  const adjustedPosition = `${adjustedX} ${adjustedY} ${adjustedZ}`;
